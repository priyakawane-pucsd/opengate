...
func (r *Repository) CreateUpdateConfig(ctx context.Context, cfg *dao.Config) error {
...
func (r *Repository) GetAllConfigs(ctx context.Context) ([]*dao.Config, error) {
	collection := r.conn.Database(r.cfg.Database).Collection(CONFIG_COLLECTION)
...
func (r *Repository) GetConfigById(ctx context.Context, id string) (*dao.Config, error) {
	// Specify the MongoDB collection and filter criteria
	collection := r.conn.Database(r.cfg.Database).Collection(CONFIG_COLLECTION)
